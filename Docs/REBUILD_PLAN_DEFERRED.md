...
| R4 Rate limiting / App Check | Done 2026-09-14 (R-04) — `ENFORCE_APP_CHECK` switch for every callable, web App Check with reCAPTCHA Enterprise, tenant custom domains synced to the key, per-user limits on email sends, PDF previews, and invitations (`rateLimits`); alongside the pay-attempt and sign-in-link limits. See REBUILD_PLAN |
