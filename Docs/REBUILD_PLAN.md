...
| D10 | **Amazon SES runs in `us-east-1`** (2026-09-17), the single exception to D2's all-compute-in-Canada rule. Everything else stays in Canada. Custom MAIL FROM is `mail.techflowsolutions.ca`; the configuration set is `techflow-transactional`. | This account's SES production access was granted in `us-east-1` (50,000/day, 14/sec). `ca-central-1` was still sandboxed (200/day, 1/sec) and completely empty — no identities, no configuration sets — so using it meant a fresh AWS review for no product benefit. TechFlow has no clients and no data-residency requirement today. The cost is real and recorded: email bodies carry customer names, addresses, and invoice amounts, so transactional email is the one place tenant data leaves Canada, and the Privacy policy (Y-06) must disclose US processing. Revisit if a client ever requires Canadian residency — SES stores no data, so moving regions is a config value, an MX record, and an SNS topic. | `functions/src/emails/send.ts` (`DEFAULT_REGION`), `SES_REGION` in `functions/.env.<projectId>`, MAIL FROM MX `feedback-smtp.us-east-1.amazonses.com`, and the SNS topic behind `sesEventsWebhook` |
...
| 3 Frontend architecture | Contexts, guards, auth recovery done; dashboard navigation and home (S-01); invoices list (S-02); new invoice (S-03); invoice detail (S-04); customers (S-05); quotes list, new quote, and quote detail (S-06); portal home (S-07); portal invoice and quote pages (S-08); public pay page with its success and cancelled pages (S-09); portal sign-in by email link (S-10) | No placeholder pages left |
...
Fixed: S-03, part 1 (2026-09-14) — `validateInvoiceInput`, `validateQuoteInput`, and `validateRecurringInvoiceInput` only checked that the customer email wasn't empty, and matched dates against `\d{4}-\d{2}-\d{2}`, so `jane.example.com` and `2026-02-31` were saved and only failed later — the email when SES refused it, the date wherever it was read. All three now use the shared `isValidEmail` (as `upsertCustomer` and invitations already did) and `isIsoCalendarDate` (`functions/src/shared/dates.ts`, which round-trips the parts through a UTC date), and refuse a due date before the issue date, a `validUntil` before the issue date, and an `endDate` before the `startDate`. The invoice form mirrors these checks (`src/lib/isoDate.ts`, `src/lib/email.ts`), pinned to the functions copies by `src/lib/__tests__/invoiceFormMirrors.test.ts`. Covered by `functions/test/shared/dates.test.ts` and the invoice, quote, and recurring callable tests.
...
Fixed: S-09, part 1 (2026-09-15) — `createPayTokenCheckoutSession` never checked the `stripePayments` entitlement the callable table lists, so a tenant whose plan doesn't include card payments but still has a connected Stripe account could be paid by card; it now refuses before any Stripe call. `verifyInvoicePayToken` couldn't give the pay page what its layout needs — no issue or due date, and nothing to say whether card payments were possible — while returning the customer's email and the raw snapshot. It now returns the pay page's own payload (see the callable table): card terms only when the tenant has `stripePayments` and a Stripe account that can take charges, and e-Transfer only on CAD invoices, since Interac e-Transfer moves Canadian dollars only. Both callables take the amount and fee from `cardChargeFor` (`functions/src/shared/payAmounts.ts`), so the page can't show one amount while Checkout charges another. Covered by `functions/test/shared/payAmounts.test.ts` and `customerFacing.test.ts`.

Fixed: S-10, part 1 (2026-09-15) — the plan says the invoice email links to `/portal/invoices/{id}?tenantId={tenantId}` on the business's custom domain when it has one, but `sendInvoiceEmail`'s portal link, and its pay link for an invoice without a pay token, went to `/portal/login` on the shared host, as did the payment receipt's (`onInvoicePaid`) and a recurring invoice's fallback; `sendQuoteEmail` linked the quote page but always on the shared host. All four now use `portalDocumentUrl` (`functions/src/shared/portalLinks.ts`): the document's portal page, on the business's custom domain once it is verified, else `APP_URL`, and signing in returns the customer to it (S-10 part 2). Covered by `functions/test/shared/portalLinks.test.ts`.
...
- Verify `techflowsolutions.ca` in SES `us-east-1` (Easy DKIM, custom MAIL FROM, existing DMARC) — see the Deploy Runbook and D10.
...
rateLimits/{uid}_{action}                  ← per-user limits: send-email 50/h, pdf-preview 60/h,
  { uid, action, windowStartedAt, count,     invitation 20/h (R-04); admin-SDK only; TTL on
    expireAt }                               expireAt
...
`firestore.rules` is authoritative, verified by 62 emulator tests in `functions/test/rules/firestore.test.ts`. The April sketch that used to live here allowed client writes to `meta`, `customers`, `recurringInvoices`, and `invitations`; the build tightened every one of them to callable-only writes.
...
| `customDomains/*`, `stripeAccounts/*`, `stripeEvents/*`, `emailSends/*`, `signups/*`, `signInLinkLimits/*`, `rateLimits/*` | none | none |
...
- **Firestore indexes deployed (`firestore.indexes.json`)** — required from day one. The `getCustomerInvoices` query uses a `collectionGroup('invoices')` query with `.where('customer.email', '==', email).orderBy('createdAt', 'desc')`. Without a composite index defined for this collection group, Firestore throws a runtime error on the first customer portal query. Known required indexes:
  ...
  - TTL field overrides (`ttl: true`, indexing disabled): `stripeEvents.expireAt`, `payAttempts.expireAt`, `emailSends.expireAt`, `signInLinkLimits.expireAt` (E-01), `rateLimits.expireAt` (R-04)
...
Region: callables, HTTP functions, and Firestore triggers run in `northamerica-northeast2`; scheduled functions in `northamerica-northeast1` (D2). Tenant callables require a `tenantId` claim (`requireTenant`); feature gates use `requireFeature`, which throws `permission-denied` and returns the resolved feature map. App Check: every callable enforces it once `ENFORCE_APP_CHECK=true` (off by default; `functions/src/shared/globalOptions.ts`, R-04). Per-user hourly limits (`functions/src/shared/rateLimit.ts`, R-04): `sendInvoiceEmail` + `sendQuoteEmail` 50 combined, `previewInvoicePDF` + `previewQuotePDF` 60 combined, `createInvitation` 20 — over the limit a call fails with `resource-exhausted` before doing any work.
...
| Function | Notes |
|---|---|
| `verifyInvoicePayToken` | Returns a discriminated `VerifyResult` (`ok` \| `paid` \| `refunded` \| `void` \| `regenerated` \| `not-available`; `void` is checked before the link version, A-12). `ok` carries only what the pay page shows (S-09): the business from the snapshot with its email footer, the invoice's dates, lines, and totals, the customer's name, `amountDueCents`, e-Transfer details when the snapshot has an e-Transfer email and the invoice is in CAD (Interac e-Transfer moves Canadian dollars only), and card terms — fee percent, fee, and total from `cardChargeFor` (`functions/src/shared/payAmounts.ts`), the snapshot surcharge behind the `cardSurcharge` kill switch (D3) — only when the tenant has `stripePayments` and a Stripe account that can take charges. `paid`, `refunded`, and `void` also carry the business, for the page's header |
| `createPayTokenCheckoutSession` | `stripePayments` of the invoice's tenant (checked since S-09); max 10 sessions per invoice per 24h; charges `cardChargeFor` — exactly what the pay page showed — as a direct charge on the tenant's Stripe account |
...
2. `validateInvoiceInput` — customer name and a valid email (`isValidEmail`, lowercased at the write boundary, C2); 1–100 line items through the shared `validateLineItems`, where each line's `taxable` defaults to `applyTax` (D4); `dueDate` and optional `issueDate` as real `YYYY-MM-DD` calendar dates (`isIsoCalendarDate`), with the due date not before the issue date; notes ≤ 2000 characters. `validateQuoteInput` and `validateRecurringInvoiceInput` apply the same email and date rules (`validUntil` not before `issueDate`; `endDate` not before `startDate`).
...
**`regenerateInvoicePayLink`** (owner/admin callable) — increments `payTokenVersion`, signs a new JWT, overwrites `payToken` and `payTokenExpiresAt`. Useful when a customer accidentally forwards the email or a tenant wants to kill an old link. `updateInvoice` does the same automatically when a sent invoice's total or customer email changes (A-08).
...
- **Configuration set** `SES_CONFIGURATION_SET` (required for bounce/complaint events). **SES tenants**: `SES_TENANTS_ENABLED=true` passes `TenantName = tenantId` once tenants are provisioned in SES — each TechFlow tenant then gets isolated reputation metrics, automatic pausing, and (with `SuppressionScope: TENANT`) its own suppression list. Required before the first real client, not optional polish: without it a bounce for one business silently blocks that address for every other business. See the Deploy Runbook, step 5.
...
- **Email verification gate** — add `if (!user.emailVerified) redirect('/verify-email')` to the `(dashboard)` layout. `/verify-email` shows a "resend verification email" button and a "check your inbox" message. Block invoice creation until verified to satisfy Stripe Connect's business-contact-email expectations. As built (S-01): `DashboardAuthGuard` applies `dashboardGuardRedirect` (`src/lib/auth/guardRedirects.ts`), which reads the Firebase `User.emailVerified` — current after `user.reload()`, unlike the token's `email_verified` claim — so a new owner goes from signup to `/verify-email`, and that page's poll continues to `/dashboard` once the link is clicked.
...
As built (S-01): the config is `DASHBOARD_NAV` in `src/lib/navigation/dashboardNav.ts` — Dashboard, Invoices (`invoices`), Quotes (`quotes`, added with its list page in S-06), Customers, Billing, Settings — and `DashboardShell` renders it as a sidebar on desktop and a scrolling bar under the header on phones, with an account menu (email, role, Sign out). Items can also be limited by role: Billing shows to owners and admins only, since only they can start Stripe onboarding, and the Stripe setup banner is hidden from staff for the same reason. Feature-gated items wait for entitlements to load. A test checks every item links to an existing page.
...
The invoices list (`/invoices`, S-02) filters by status through links — All, Outstanding, Overdue, Drafts, Paid, Refunded, Void — so a filter is a shareable URL (`/invoices?status=overdue`, which the dashboard links to). Each filter is one live query, `status in [...]` newest first on the same `(status, createdAt desc)` index and pinned in `firestoreIndexes.test.ts`; Overdue narrows the payable invoices by due date on the client (`src/lib/invoices/listFilters.ts`). Pages load 50 at a time. "Load more" raises the listener's limit, which re-reads the rows already shown — negligible at tenant scale, and it keeps the whole list live. Firestore has no text search, so the search box matches customer name, email, or invoice number (ignoring case and accents) across the invoices loaded, and says so while older ones remain. The dashboard and the list share `InvoiceTable`: on a phone the invoice number sits under the customer and the status under the amount.

New invoice (`/invoices/new`, S-03, `src/components/invoices/NewInvoiceForm.tsx`) picks a saved customer or takes a new one (with "Save to my customers", which calls `upsertCustomer` after the invoice is created), dates defaulting to today and 30 days later, and up to 100 lines. Each line has its own tax checkbox (D4); a header checkbox applies tax to every line and shows a mixed state when lines differ, and the tax controls disappear when the tenant has no tax rate. The totals preview uses `computeDraftTotals`, and the email and date checks use client copies of the Cloud Functions rules — all three pinned to the functions versions by `invoiceFormMirrors.test.ts`; the server still recomputes totals on save. Enter in a field saves a draft; "Save and send" (`createInvoice` then `sendInvoiceEmail`) is a separate click and is disabled, with a link to payment settings, until the tenant has an e-Transfer email or Stripe can take charges — the same precondition `sendInvoiceEmail` enforces. If sending fails after the invoice is saved, the invoice stays a draft and the toast says so. Labels are tied to their inputs directly rather than through the shared `FormControl`, which puts the label's target and `aria-invalid` on a wrapper `div`.

Invoice detail (`/invoices/[id]`, S-04, `src/components/invoices/InvoiceDetail.tsx`) reads the invoice live through `useTenantDoc` and offers only the actions the callables allow for its status and the viewer's role (`invoiceActionsFor`, whose voidable list is pinned to the functions copy). Send invoice (a draft) and Resend (while something is payable) are disabled, with a link to payment settings, until the tenant can get paid. Copy pay link works while the invoice is payable; the link also shows read-only in a Pay link card with its expiry and, for owners and admins, Issue a new link. Preview PDF renders inline from an object URL with Download and Open links rather than a popup — browsers block `window.open` after an async call, and Brave ignores blob-URL popups. Owners and admins also get Mark as paid (e-Transfer, cash, or other, with an optional receipt email), Void (with an optional reason), and Delete draft. The page marks tax-exempt lines when an invoice mixes both kinds, shows one row per tax, card fees, the balance due on a partial invoice, and an activity list — created, sent, paid, refunded, disputed, voided — with the last email delivery status. An id that isn't a valid document id shows not found without querying Firestore.

Customers (`/customers`, S-05, `src/components/customers/`) lists saved customers live — up to 500, sorted by name ignoring case and accents, since Firestore orders by code point — with search across name, email, phone, and address (the same accent-insensitive matcher as the invoices list, `src/lib/search.ts`). Add and Edit open one dialog that calls `upsertCustomer` with the same limits as `validateCustomerInput` (pinned to its source by a test); when another customer already uses the email it says so but still saves, because one contact can pay for several customers. Each row's Actions menu offers New invoice — `/invoices/new?customer=<id>`, which picks that customer in the form — Edit, and, for owners and admins, Delete (`deleteCustomer`; issued invoices and quotes keep their own copy). New forms use `src/components/forms/fields.tsx`, whose labels, hints, and errors point at the input itself.

Quotes (S-06) have three screens; the route tree above only planned the detail page, but the menu's `/quotes` item needed a list and nothing created quotes. `/quotes` mirrors the invoices list — All, Drafts, Sent, Converted, on the quotes `(status, createdAt desc)` index pinned in `firestoreIndexes.test.ts` — and nothing stores `expired`, so a sent quote past its `validUntil` shows as Expired (`src/lib/quotes/quoteStatus.ts`). `/quotes/new` and `/invoices/new` are one form (`src/components/documents/NewDocumentForm.tsx`, `kind` quote or invoice): a quote has a valid-until date instead of a due date, calls `createQuote` and `sendQuoteEmail`, and needs no payment setup to send. `/quotes/[id]` offers Send or Resend, Preview PDF, Convert to invoice — a dialog that sends the viewer's issue date and a due date defaulting to 30 days (`convertQuoteToInvoice`, S-06 part 1), then opens the new draft invoice — and, for owners and admins, Delete. A converted quote links to its invoice and offers only the PDF.

The portal home (`/portal`, S-07, `src/app/(portal)/portal/PortalHome.tsx`) groups what `CustomerPortalContext` loads, in `src/lib/portal/portalHome.ts` (its status lists pinned to `customerVisibility.ts` by a test): Total to pay per currency with the overdue share, never added across currencies; To pay, past due first and then soonest due, where a partly paid invoice shows its balance and what was paid; Open quotes, soonest to expire first; then Past invoices and Past quotes, newest first. Overdue and expired are derived from the viewer's date, as on the dashboard. Each row shows the sending business's logo — or its initial on its brand colour, used only when it is a `#rrggbb` hex — from that document's snapshot, and links to `/portal/invoices/[id]?tenantId=…` or `/portal/quotes/[id]?tenantId=…`. A customer with nothing yet sees an empty state; when a list fails to load, the page says so with Try again and still shows the other list. `PortalShell` adds a neutral header with Sign out — no business or platform name, so it reads right on any custom domain.

The portal invoice and quote pages (`/portal/invoices/[id]` and `/portal/quotes/[id]`, S-08) take the business from `?tenantId` and load the customer's view through `getCustomerInvoiceDetail` or `getCustomerQuoteDetail` (`src/components/portal/PortalDocument.tsx`; the rules are in `src/lib/portal/portalDocument.ts`, whose view types a type test pins to the functions copy). Each reads like the paper copy in the sending business's branding — logo or initial, address, business number, items, one line per tax, totals, notes. An invoice leads with the amount due (the balance, on a partly paid one), when it's due or how long overdue, and a Pay button in the business's colour, with its text colour picked for contrast, that opens `/pay/{payToken}` — the page the email links to — without prefetching it. When the pay link has expired the page says so and asks the customer to have the business send the invoice again. Paid, refunded, and cancelled invoices show that outcome instead, never the business's void reason. A quote shows its total and valid-until date and, since customers can't accept in the portal yet, asks them to reply to the business's email; expired, accepted, declined, and converted quotes say so. Get PDF fetches the PDF with the customer's ID token and then offers Download and Open links the customer taps: Safari may ignore a download started from script after an await, and iOS shows only the first page of a PDF in a frame, so there is no automatic download or embedded preview. In-app browsers built on WKWebView, as some mail apps use, can't open those links. A draft, someone else's document, an unknown business, and a malformed link all read as not found, and a malformed link is never requested. The tab takes the business's name and favicon from the custom domain or `?tenantId` (`src/lib/tenant/portalMetadata.ts`), and neither page is indexed.

The public pay page (`/pay/[token]`, S-09, `src/app/(pay)/pay/[token]/PayInvoice.tsx`; building blocks in `src/components/pay/PayFrame.tsx`, rules in `src/lib/pay/payPage.ts`, whose types a type test pins to the callable's) checks its link from the browser (D8) and follows the render order in "Public pay page": the business's colour band with its logo and name, and a footer with its address, email footer, and "Questions? Reply to the email that brought you here." — no platform branding. The summary shows the invoice number, the amount, who it's billed to, the issue and due dates (overdue in red), and an Itemized view collapsed in a native `details` element. How to pay lists e-Transfer first — the send-to address, the amount, the invoice number for the message, Copy details, and, above $3,000, a note that most Canadian banks cap a single e-Transfer there — then Card, whose button in the business's colour starts Checkout through `createPayTokenCheckoutSession` and shows the callable's own reason when it refuses. The two options are separate cards, each with its own button, rather than the radio selector sketched in the spec, so paying by card stays one click. When a card fee applies it is stated before payment as a percentage and in dollars, with the card total, as the card networks require, and e-Transfer is labelled No card fee; a business with neither option asks the customer to contact it. Paid, refunded, cancelled, replaced, and unavailable invoices each get their own message, and a link that fails verification reads as not valid. `/pay/[token]/success` confirms the payment — the Stripe webhook records it, never this page — and if it hasn't landed checks once more three seconds later, then says the payment is being confirmed and a receipt will follow; on a replaced link it explains the automatic refund. `/pay/[token]/cancelled` says no charge was made and links back to the invoice. Customer-facing reads — checking the pay link, the portal lists, and the portal invoice and quote pages — give up after 20 seconds (`CUSTOMER_READ_TIMEOUT_MS` in `src/lib/firebase/callOptions.ts`) and offer Try again, rather than waiting out the Functions SDK's 70-second default on weak signal; starting Checkout and sending a sign-in link keep the default, because the server may still finish them after the page stops waiting. Pay links — in invoice and recurring-invoice emails, in PDFs, and in the dashboard's Copy pay link — and Stripe's success and cancel URLs use the business's verified custom domain, else the shared portal host (`payPageUrl` in `functions/src/shared/portalLinks.ts`, `portalOriginFor` in `src/lib/tenant/portalOrigin.ts`), so a customer who starts on the business's address stays on it through Checkout and back.

Portal sign-in is by emailed link only (S-10) — customers have no password. The portal guard sends a signed-out visitor to `/portal/login?next=<the page they asked for>` (`portalGuardRedirect` in `src/lib/auth/guardRedirects.ts`, whose `safePortalReturnPath` keeps only same-site `/portal` pages and never the login itself); it used to drop the page. The login (`src/app/portal/login/`) takes the business's branding from the custom domain or from the `tenantId` inside `next`, stores the address in `localStorage` as `emailForSignIn` before calling `sendPortalSignInLink`, and answers the same way whether or not the address has documents. The link's continue URL is `/portal/login?next=…` rather than the document page the flow above names: the login sits outside the portal guard, so it can finish signing in before anything checks for a user, and then opens `next`. Opened in the browser that asked for it, the link signs in at once (`isSignInWithEmailLink`, `signInWithEmailLink`) and clears the stored address; opened elsewhere, the page asks for the address, which Firebase requires and warns must never travel in the URL. A different address is told "That isn't the email address this sign-in link was sent to"; an expired or used link says so and offers a new one. A customer already signed in who reaches the login goes straight to `next`. Password sign-in stays on the main `/login` for tenant users, which sends a verified customer on to the portal.
...
- `checkout.session.completed` — `metadata.tenantId` must match the routed tenant (otherwise a `tenant-mismatch` incident); a session whose `payment_status` isn't `paid` is logged and ignored (no funds captured). One transaction then checks the payment against the invoice, in order: the invoice must exist and be payable (`sent`/`unpaid`/`overdue`/`partial`) — otherwise `auto-refund-not-payable`, or `auto-refund-duplicate-payment` when it already holds a different payment (the same PaymentIntent redelivered is a no-op); **C2:** `metadata.payTokenVersion` must equal the invoice's current version — otherwise `auto-refund-version-mismatch`; **A-08:** `basePaidCents` must equal the invoice total in cents, `amount_total` must equal base + surcharge, and the currency must match the snapshot — otherwise `auto-refund-amount-mismatch`. A failed check refunds on the connected account (idempotency key per session; reason `duplicate` for duplicates) and writes the incident, which emails owners; a transient Stripe error instead fails the event so Stripe retries it. A passing payment sets `status: 'paid'`, `paidAt`, `paymentMethod: 'card'`, `paidAmountCents`, `surchargeAmountCents`, `stripePaymentIntentId`, `stripeCheckoutSessionId`.
...
- **Next.js proxy** (`src/proxy.ts` — Next 16's name for middleware; it always runs on the Node.js runtime):
  ...
  **Required — Vercel Edge Config as the authoritative lookup, Firestore as the write-side source:**
  1. The `setupCustomDomain` Cloud Function (the same one that manages Vercel Domains API + Firebase authorized domains, and the App Check reCAPTCHA key's allowed domains when `RECAPTCHA_KEY_ID` is set — R-04) also writes `{ [domainCacheKey(domain)]: tenantId }` to Vercel Global Config (Vercel's 2026 name for Edge Config) via `PATCH /v1/global-config/{id}/items`. Keys must match `^[\w-]+$` (≤ 256 chars), so `domainCacheKey` encodes the host as `domain_` + host with `.` → `_` (A-07).
...
8. **Custom domain verification (if Phase 5 custom domains are complete):**
   - Set `customDomain` on a test tenant → Cloud Function adds Vercel domain + Firebase Auth authorized domain + reCAPTCHA key allowed domain (R-04) + `customDomains/{domain}` doc
...
Defaults follow D9: every plan includes every feature except card surcharging. A tenant's `entitlements/current.features` can still set any key to `false`. When adding an advanced feature meant for a premium plan (AI invoicing, scheduling, accounting sync), add a new key with default `false` to both copies of `FEATURE_DEFAULTS` and gate it like the others — never put it behind an existing key.
...
| Amazon SES | `us-east-1` (D10) — the region that holds this account's production access; the one exception to D2 |
...
| `NEXT_PUBLIC_RECAPTCHA_ENTERPRISE_SITE_KEY` | App Check in the browser (R-04); unset leaves App Check off. The key must list the platform domain; tenant custom domains are added by `setupCustomDomain` |
...
| `NEXT_PUBLIC_APP_URL` | pay links in PDFs and the dashboard's Copy pay link (`https://portal.techflowsolutions.ca` in prod); a business's verified custom domain replaces it (`src/lib/tenant/portalOrigin.ts`) |
...
|---|---|---|
| `APP_URL` | `https://portal.techflowsolutions.ca` | links in emails, Stripe redirect URLs, invitation accept URL; for a business with a verified custom domain, that domain replaces it in customers' portal and pay links and Stripe redirects (`functions/src/shared/portalLinks.ts`) |
| `PDF_SERVICE_URL` | — | the deterministic Cloud Run URL (below) |
| `SES_REGION` | `us-east-1` | D10. Must match the region holding production access; `ca-central-1` is still sandboxed on this account |
| `EMAIL_FROM_ADDRESS` | `notifications@techflowsolutions.ca` | must belong to a verified SES identity |
| `SES_CONFIGURATION_SET` | — | required for bounce/complaint events. If it is missing, `send.ts` still sends — refusing to invoice over a telemetry setting is worse than the gap — but logs an error on **every** send, so the omission reaches Sentry (O-01) instead of passing unnoticed. Without it SES publishes no events at all, so a dead address is indistinguishable from a delivered one |
...
- Firestore TTL policies (`payAttempts`, `stripeEvents`, `emailSends`, `signInLinkLimits`, `rateLimits` on `expireAt`) are declared in `firestore.indexes.json` and created by the indexes deploy below — nothing to set by hand. Expired documents are typically deleted within 24 hours.
...
- Set the functions secrets and `functions/.env.<projectId>`, then `firebase deploy --only functions --project <projectId>`. The CLI first loads the code to discover its functions and gives up after 10 seconds (all 44 load in about 2 seconds locally). If it reports `User code failed to load. Cannot determine backend specification. Timeout after 10000` on a cold or busy machine, retry, or set `FUNCTIONS_DISCOVERY_TIMEOUT=30` for that deploy — the override Firebase documents in its Cloud Functions tips.
...
- App Check (R-04), in this order:
  1. In Google Cloud, enable the reCAPTCHA Enterprise API and create a **Website** key (score-based, no checkbox challenge) listing the platform domain — `techflowsolutions.ca` also covers its subdomains. Never add `localhost` to a production key; use a separate test key or the App Check debug provider (`NEXT_PUBLIC_APP_CHECK_DEBUG_TOKEN`, development and CI only).
  2. Register the web app in Firebase Console → Security → App Check with that key. Put the key in `NEXT_PUBLIC_RECAPTCHA_ENTERPRISE_SITE_KEY` (Vercel) and its id in `RECAPTCHA_KEY_ID` (`functions/.env.<projectId>`), and grant the functions service account `recaptchaenterprise.keys.get` and `recaptchaenterprise.keys.update` so `setupCustomDomain` can add tenant domains to the key. Existing custom domains must be added to the key by hand once.
  3. Deploy both, then watch the App Check metrics until the app's own requests show as verified. Only then set `ENFORCE_APP_CHECK=true` and redeploy functions — turning it on earlier rejects every call.
  - Limits: a key lists at most 250 domains, and App Check doesn't support a web app on more than 250 domains, so custom domains top out near 250 tenants. Each token refresh is a reCAPTCHA assessment, billed beyond the free quota.
...
- **Before the first real client: one SES tenant per TechFlow tenant**, each with the identity and configuration set associated, then `SES_TENANTS_ENABLED=true` (which makes `send.ts` pass `TenantName = tenantId`). Set each tenant's `SuppressionScope: TENANT` with `SuppressedReasons: BOUNCE, COMPLAINT` — via `PutTenantSuppressionAttributes`, or inline at `CreateTenant`. Without it every tenant shares the account-level suppression list, so one business's bounced customer silently blocks that same address for *every other* business: SES accepts the message, never delivers it, and reports no error. Tenants also isolate reputation — SES and AWS Trust & Safety can pause a single bad tenant instead of the whole account.
  - Suppression precedence is **configuration set → tenant → account**. Never set suppression options on `techflow-transactional`, or the config set overrides tenant scope and re-shares one list across all tenants.
  - A send blocked this way returns bounce type `Permanent` with subtype `OnTenantSuppressionList`, and events carry a `ses:tenant-name` tag — that is how to tell it apart from a real bounce.
  - Proven on this account: the `cheap-cheap-cheap` tenant has run since 2026-06 for a client site, so the mechanism is in use, not theoretical.
